...
def title(title_text):
    global _console_title
    _console_title = title_text
    if os.name == "nt":
        import ctypes
        ctypes.windll.kernel32.SetConsoleTitleW(title_text)
    else:
        # ANSI escape to set terminal title (works in most terminals / Termux)
    ...
    def main_menu(self):
        ...
        def check(word):
            if os.name == "nt":
                ...
                # On Linux/Termux, read back from the tracked module-level variable
                return word.lower() in _console_title.lower()
